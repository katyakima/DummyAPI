...
Стоит отметить, что тест **Check body likes** падает. Это связано с тем, что в тесте есть запрос на изменение количества лайков самим пользователем (owner), что в условиях настоящей социальной сети должно быть недопустимо. Создатель поста не должен иметь возможность ставить своему посту лайки сам.

### DELETE /post/:id (DELETE Post)

Создано 4 теста с проверкой статус-кода, времени ответа и тела ответа, в котором должен присутствовать id удаленного пользователя.

![Ответы для запроса DeletePost](https://i.imgur.com/iLP8GZm.jpeg)
